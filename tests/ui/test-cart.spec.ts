...
test.describe('Cart Tests', () => {
  ...
  test('TC01 - Verify Empty Cart @regression', async ({ cartPage }) => {
  ...
  test('TC02 - Add Product to Cart @smoke @regression', async ({ productPage, cartPage }) => {
  ...
  test('TC03 - Remove Product from Cart @regression', async ({ productPage, cartPage }) => {
    await productPage.commonPage.goto(Constants.BASE_URL);
    await productPage.searchAndSelectProduct(product);
    await productPage.clickAddToCart();
    await productPage.verifyAddToCartSuccessMessage(Messages.ADD_TO_CART_SUCCESS_MESSAGE);
  ...
  test('TC04 - Update Product Quantity @regression', async ({ productPage, cartPage }) => {
    await productPage.commonPage.goto(Constants.BASE_URL);
    await productPage.searchAndSelectProduct(product);
    await productPage.clickAddToCart();
  ...
  test('TC05 - Update Product Quantity to 0 (Remove via Quantity) @regression', async ({ productPage, cartPage }) => {
    await productPage.commonPage.goto(Constants.BASE_URL);
    await productPage.searchAndSelectProduct(product);
    await productPage.clickAddToCart();
    await productPage.verifyAddToCartSuccessMessage(Messages.ADD_TO_CART_SUCCESS_MESSAGE,);
